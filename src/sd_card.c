...
HAL_StatusTypeDef Calc_CRC16(
    ...
    bool building_crc
        ...
        if (byte_count < 2 || response_adr == NULL || data == NULL || (pattern & 0x80 != 0x80)) {
            return HAL_ERROR;
        }
        uint16_t cur_data = (data[0] << 8) | data[1];
        for (uint32_t i = 2; i < byte_count; i++) {
            for (uint8_t j = 0; j < 8; j++) {
                bool bit15_set = (cur_data & 0x8000) == 0x8000;
                cur_data = cur_data << 1 | (data[i] & (uint16_t)1 << j);
                if (bit15_set) {
                    cur_data ^= pattern;
                }
            }
...
HAL_StatusTypeDef Send_Command(Card_Handle *card, uint8_t cmd, uint32_t cmd_arg, uint8_t *R1_adr) {
    ...
    // transmission loop, this can be repeated up to 8 times in the case of crc errors.
    ...
    if (attempt_count == 8) return HAL_TIMEOUT;
...
HAL_StatusTypeDef Get_Sector_Count(Card_Handle *card) {
    ...
    uint8_t cmd9_arg[4];
    uint8_t R1_response;
    memset(cmd9_arg, 0, sizeof(cmd9_arg));

    status = Send_Command(card, cmd9, cmd9_arg, &R1_response);
    if (status != HAL_OK) return status;
    if (R1_response != 0) return HAL_ERROR;
    ...
    uint32_t c_size = (((uint8_t)CSD_reg[7] & 0x3f) << 16) | ((uint8_t)CSD_reg[8] << 8) | (uint8_t)CSD_reg[9];
